...
function isRealtimeEnabledNow() {
  // realtime only 21:00–22:00 Manila
  const { hh, mm } = manilaNow();
  return hh === 21 || (hh === 22 && mm === 0); // allow exact 22:00 edge
}
...
export async function GET(req: Request) {
  ...
  if (realtime_enabled) {
    ...
    if (!bdErr && Array.isArray(bd)) {
      ...
      // keep null; UI will show nothing (or your warning banner if you added it)
  ...
  // ✅ IMPORTANT: always return realtime_breakdowns (even null)
  return NextResponse.json({
    ok: true,
    now: nowIso,
